...
def _mirror_tooth_fdi(tooth: int) -> int:
    q = tooth // 10
    u = tooth % 10
    # Espelho sagital em FDI: 1<->2, 4<->3
    q_map = {1: 2, 2: 1, 3: 4, 4: 3}
    q2 = q_map.get(q, q)
    return (q2 * 10) + u


def _build_mirror_label_map(label_to_idx: dict[str, int]) -> dict[int, int]:
    labels = list(label_to_idx.keys())
    token_map: dict[str, list[int]] = {}
    for lbl in labels:
        toks = [int(x) for x in lbl.split("-")]
        token_map[lbl] = toks

    by_tokens: dict[tuple[int, ...], str] = {tuple(v): k for k, v in token_map.items()}
    by_set: dict[frozenset[int], list[str]] = {}
    for k, v in token_map.items():
        by_set.setdefault(frozenset(v), []).append(k)

    result: dict[int, int] = {}
    for lbl, idx in label_to_idx.items():
        toks = token_map[lbl]
        mirrored = [_mirror_tooth_fdi(t) for t in toks]

        # 1) tentativa direta preservando ordem
        cand = by_tokens.get(tuple(mirrored))
        # 2) tentativa reversa (caso classes usem ordem anatômica oposta)
        if cand is None:
            cand = by_tokens.get(tuple(reversed(mirrored)))
        # 3) fallback por conjunto (somente se único)
        if cand is None:
            cands = by_set.get(frozenset(mirrored), [])
            if len(cands) == 1:
                cand = cands[0]
        # 4) se ainda não achou, mantém a própria classe (seguro)
        if cand is None:
            cand = lbl

        result[idx] = label_to_idx[cand]
...
def _extract_split_features(
    ...
    for batch in loader:
        ...
        if augment_flip_mirror:
            flipped = torch.flip(pixel_values, dims=[3])
            feats_flip = backbone._extract_features(flipped).detach().cpu().numpy().astype(np.float32)
            if mirror_label_idx_map is None:
                labels_flip = labels.copy()
            else:
                labels_flip = np.array(
                    [int(mirror_label_idx_map.get(int(l), int(l))) for l in labels],
                    dtype=np.int64,
                )
            feats_list.append(feats_flip)
            labels_list.append(labels_flip)
...
def run_training_cached(args: Namespace) -> None:
    ...
    use_flip_mirror = bool(args.augment_flip_mirror)
    mirror_label_idx_map = _build_mirror_label_map(label_to_idx) if use_flip_mirror else {}
    ...
    expected_meta = {
        ...
        "mirror_label_idx_map": {str(k): int(v) for k, v in mirror_label_idx_map.items()},
    }
    feature_paths = {
