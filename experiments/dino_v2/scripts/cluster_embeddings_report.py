...
def _resolve_listed_path(raw_path: str, root_dir: Path | None, cwd_dir: Path) -> Path:
    p = Path(raw_path)
    if p.is_absolute():
        return p
    if root_dir is not None:
        return (root_dir / p).resolve()
    return (cwd_dir / p).resolve()


def _load_images_from_json_list(list_json_path: Path, cwd_dir: Path) -> list[Path]:
    ...
    if isinstance(payload, list):
        ...
        if isinstance(payload.get("samples"), list):
            ...
            raw_items = payload["paths"]
        else:
            raise SystemExit(
                "JSON de lista invalido: esperado list ou dict com samples/items/paths."
            )
    else:
        raise SystemExit("JSON de lista invalido: formato nao suportado.")
    ...
    for it in raw_items:
        ...
        if isinstance(it, str):
            ...
            for key in ("path", "image", "image_path"):
                if it.get(key):
                    raw_path = str(it[key])
                    break
        if not raw_path:
            continue
        p = _resolve_listed_path(raw_path, root_dir=root_dir, cwd_dir=cwd_dir)
    ...
    if not out:
        raise SystemExit(f"Nenhuma imagem valida encontrada no JSON: {list_json_path}")
    return sorted(out)
...
def _make_image_symlink(src_image: Path, images_dir: Path | None, linked_images_dir: Path) -> Path:
    rel: Path | None = None
    if images_dir is not None:
        try:
            ...
            rel = None
    if rel is None:
        stem = _sanitize_name(src_image.stem)
        folder = _sanitize_name(src_image.parent.name)
        h = hashlib.sha1(str(src_image.resolve()).encode("utf-8")).hexdigest()[:12]
        rel = Path(f"{folder}__{stem}__{h}{src_image.suffix.lower()}")
...
def main() -> None:
    ...
    parser.add_argument("--images-dir", default="", help="Pasta com imagens. Ignorado se --images-list-json for usado.")
    parser.add_argument("--images-list-json", default="", help="JSON com lista de imagens a usar (gerado por script de amostragem).")
    ...
    if args.images_list_json:
        list_json_path = Path(args.images_list_json)
        if not list_json_path.exists():
            raise SystemExit(f"JSON de lista não encontrado: {list_json_path}")
        image_paths = _load_images_from_json_list(list_json_path, cwd_dir=Path.cwd())
    else:
        if not args.images_dir:
            raise SystemExit("Informe --images-dir ou --images-list-json")
    ...
    for idx, path in enumerate(image_paths, start=1):
        if path.name.startswith("._") or "__MACOSX" in path.parts:
            skipped_invalid_images.append(str(path))
    ...
    if not embeddings:
        raise SystemExit("Nenhum embedding valido foi extraido (todas as imagens foram ignoradas/falharam).")
    ...
    timing = {
        ...
        "num_images_requested": int(len(image_paths)),
        "num_images_skipped_invalid": int(len(skipped_invalid_images)),
    ...
    summary = {
        ...
        "num_images_skipped_invalid": len(skipped_invalid_images),
    ...
    if skipped_invalid_images:
        summary["skipped_invalid_examples"] = skipped_invalid_images[:50]
