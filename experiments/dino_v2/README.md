...
## 6b) Gerar JSON com N amostras por subpasta
```bash
python scripts/build_sample_list_from_subfolders.py \
  --root-dir /dataminer/radiobot/alguma_pasta \
  --n-per-folder 100 \
  --seed 42
```

Isso cria, por padrão, um JSON em:
- `/dataminer/radiobot/alguma_pasta/sample_list_n100.json`

## 6c) Rodar clusterização usando lista JSON
```bash
PYTHONPATH=src python scripts/cluster_embeddings_report.py \
  --model-id facebook/dinov2-small \
  --images-list-json /dataminer/radiobot/alguma_pasta/sample_list_n100.json \
  --output-dir outputs/cluster_from_json \
  --n-clusters 8
```
